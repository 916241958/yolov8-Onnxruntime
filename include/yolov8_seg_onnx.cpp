...
//		_inputName = std::move(_OrtSession->GetInputNameAllocated(0, allocator));
        _inputName = static_cast<const shared_ptr<char>>(std::move(_OrtSession->GetInputName(0, allocator)));
...
//		_output_name1 = std::move(_OrtSession->GetOutputNameAllocated(1, allocator));
        _output_name0 = static_cast<const shared_ptr<char>>(std::move(_OrtSession->GetOutputName(0, allocator)));
        _output_name1 = static_cast<const shared_ptr<char>>(std::move(_OrtSession->GetOutputName(1, allocator)));
#endif
